...
// 从目标数据表中查询数据值
BOOL CSqliteManager::SelectData(CString TableName, CString &pResult, CString Column, CString Params, bool Console, CString Order, CString Limit, int SortMode, BOOL DISTINCT, CString COUNT, CString COLUMN, CString GROUP, CString HAVING)
{
	// 链表对象
	vector<CString> Values;

	// 查询数据
	if(! SelectData(TableName, Values, Params, Console, Order, Limit, SortMode, DISTINCT, COUNT, COLUMN, GROUP, HAVING) )
	{
		// 提示错误信息
		if(!Console)
			AfxMessageBox(_T("查询数据操作失败!"));
...
// 在目标数据表中检查数据值
BOOL CSqliteManager::CheckData (CString TableName, CString Column, CString Params, bool Console, CString Order, CString Limit, int SortMode, BOOL DISTINCT, CString COUNT, CString COLUMN, CString GROUP, CString HAVING)
{
	// 若数据库未连接成功
	if (!IsConnect)
	{
	...
	else
	{
		...
		if (SelectData(TableName, CheckData, Column, Params, Console, Order, Limit, SortMode, DISTINCT, COUNT, COLUMN, GROUP, HAVING))
		{
			if (!CheckData.IsEmpty() && CheckData != _T(""))
			{
				// 返回目标数据存在
...
// 从目标数据表中求数据项和
BOOL CSqliteManager::SumData(CString TableName, CString Column, CString Params, double &SumData, bool Console)
{
	// 链表对象
...
// 从目标数据表中求数据项积
BOOL CSqliteManager::ProductData(CString TableName, CString Column, CString Params, double &ProductData, bool Console)
{
	// 链表对象
	vector<CString> Result;
...
// 从目标数据表中求数据项平均数
BOOL CSqliteManager::AvgData(CString TableName, CString Column, CString Params, double &AvgData, bool Console)
{
	// 链表对象
	vector<CString> Result;
...
// 从目标数据表中求数据项众数
BOOL CSqliteManager::PluData(CString TableName, CString Column, CString Params, double &PluData, bool Console)
{
	// 链表对象
	vector<CString> Result;
...
// 从目标数据表中求数据项中位数
BOOL CSqliteManager::MidData(CString TableName, CString Column, CString Params, double &MidData, bool Console)
{
	// 链表对象
	vector<CString> Result;
	vector<double>  Values;
...
// 从目标数据表中求数据项最大值
BOOL CSqliteManager::MaxData(CString TableName, CString Column, CString Params, double &MaxData, bool Console)
{
	// 链表对象
	vector<CString> Result;
	vector<double>  Values;
...
// 从目标数据表中求数据项最小值
BOOL CSqliteManager::MinData(CString TableName, CString Column, CString Params, double &MinData, bool Console)
{
	// 链表对象
	vector<CString> Result;
	vector<double>  Values;
...
// 从目标数据表中取数据项随机数
BOOL CSqliteManager::RandData(CString TableName, CString Column, CString Params, int &RanData, bool Console)
{
	// 链表对象
	vector<CString> Result;
	vector<int>  Values;
	...
	double Min, Max;
	MinData(TableName, Column, Params, Min, Console);
	MaxData(TableName, Column, Params, Max, Console);
